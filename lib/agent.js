class Agent {
    constructor() {
        ...
        this.brain.add(tf.layers.dense({
            units: 2,
            inputShape: [8]
        }));
        this.mutationMean = 0;
    ...
    reproduce() {
        ...
        if (this.health > this.starthealth * 2) {
            ...
            let kernel_noise = tf.randomNormal(weights[0].shape, this.mutationMean, this.mutationStd);
            let bias_noise = tf.randomNormal(weights[1].shape, this.mutationMean, this.mutationStd);
            weights[0] = weights[0].add(kernel_noise);
            weights[1] = weights[1].add(bias_noise);
    ...
    think() {
        ...
        let pred = this.brain.predict(info);
        let data = pred.dataSync();
        let force = createVector(data[0], data[1]);
    ...
    display() {
        ...
        if (debug.checked()) {
            this.eyes.display();
            let v = this.velocity.copy().div(this.maxspeed).mult(this.eyes.sensorLength);
            stroke(200, 200, 0);
            strokeWeight(2);
            line(0, 0, v.x, v.y);
        }
